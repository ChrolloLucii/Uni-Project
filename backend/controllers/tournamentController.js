...
const TournamentController = {
  ...
  async updateTournament(req, res){
    try{
      ...
      const updatedTournament = await TournamentServiceApp.updateTournament(tournamentId, updatedData);
      return res.status(200).json(updatedTournament);
    } catch(error){
      return res.status(400).json({error: error.message});
    }
  ...
  async updateTeams(req, res) {
    try {
      ...
      const updatedTournament = await TournamentServiceApp.updateTeams(tournamentId, teams);
  ...
  async updateMatches(req, res){
    try{
      ...
      const updatedTournament = await TournamentServiceApp.updateMatches(tournamentId, matches);
  ...
  async disqualifyTeam(req,res){
    ...
    const updatedTournament = await TournamentServiceApp.disqualifyTeam(tournamentId, teamId);
  ...
  async getTournament(req,res){
    try{
      const {tournamentId} = req.params;
      const tournament = await TournamentServiceApp.getTournamentById(tournamentId);
