...
const TournamentServiceApp = {
	...
	async addTeamToTournament(tournamentId, teamData) {
		try {
			const tournament =
				await tournamentServiceInstance.tournamentRepository.getById(
					Number(tournamentId)
				)
			if (!tournament) {
			...
			const updatedTournament =
				await tournamentServiceInstance.addTeamToTournament(
					tournament,
					teamData
				)
	...
	async generateMatches(tournamentId) {
		try {
			...
			const updatedTournament = await tournamentServiceInstance.generateMatches(
				tournament
			)
			return updatedTournament
		} catch (error) {
	...
	async advanceRound(tournamentId) {
		try {
			...
			const updatedTournament = await tournamentServiceInstance.advanceRound(
				tournament
			)
			return updatedTournament
		} catch (error) {
	...
	async updateTeams(tournamentId, teams) {
		try {
			...
			tournament.matches =
				tournamentServiceInstance.matchService.generateMatches(
					teams,
					tournamentServiceInstance.matchFactory
				)

			const updatedTournament = await tournamentServiceInstance.update(
				tournament
			)
			return updatedTournament
		} catch (error) {
	...
	async updateMatches(tournamentId, matches) {
		try {
			...
			const updatedTournament = await tournamentServiceInstance.updateMatches(
				tournament,
				matches
			)
			return updatedTournament
	...
	async getTournamentById(tournamentId) {
		try {
			const tournament = await tournamentServiceInstance.getTournamentById(
				Number(tournamentId)
			)
			return tournament
		} catch (error) {
	...
	async assignJudge(tournamentId, judgeData) {
		try {
			const updatedTournament = await tournamentServiceInstance.assignJudge(
				tournamentId,
				judgeData
			)
			return updatedTournament
